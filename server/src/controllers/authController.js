...
import { authSchema, userSchema, isAdult } from '../utils/validation.js';

const cookieOptions = (rememberMe = false) => ({ httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production', ...(rememberMe ? { maxAge: 30 * 24 * 60 * 60 * 1000 } : {}) });
const publicUser = (user) => ({ _id: user._id, name: user.name, username: user.username, gender: user.gender, dateOfBirth: user.dateOfBirth, lastLogin: user.lastLogin });
